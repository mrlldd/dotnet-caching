...
namespace mrlldd.Caching.Tests.Caches
{
    public abstract class CacheStoreCallTestFixture : CachingTest
    {
        ...
        [Test]
        public Task GetsFromStoreAsync()
        {
            return Container
                ...
                .Effect(c => WithExactOperationsCount(() =>
                {
                    ...
                    Expression<Func<ICacheStore<InMoq>, Result>> setup = x =>
                        x.Set(It.Is<string>(s => s == EntryKey), It.Is<VoidUnit>(u => u == unit),
                            It.Is<CachingOptions>(o => o == CachingOptions), It.IsAny<ICacheStoreOperationOptions>());
                    mock.Setup(setup)
                        .Returns(Result)
                        .Verifiable();
                    var cache = c.GetRequiredService<ICache<VoidUnit, InMoq>>();
        ...
        public void RefreshesInStore()
        {
            Container
                .Effect(c => WithExactOperationsCount(() =>
                {
                    ...
                    var mock = c.GetRequiredService<Mock<ICacheStore<InMoq>>>();
                    Expression<Func<ICacheStore<InMoq>, Result>> setup = x =>
                        x.Refresh(It.Is<string>(s => s == EntryKey), It.IsAny<ICacheStoreOperationOptions>());
                    mock.Setup(setup)
                        .Returns(Result)
                        .Verifiable();
                    var cache = c.GetRequiredService<ICache<VoidUnit, InMoq>>();
        ...
        [Test]
        public Task RefreshesInStoreAsync()
        {
            return Container
                .EffectAsync(c => WithExactOperationsCountAsync(async () =>
                {
                    ...
                    var mock = c.GetRequiredService<Mock<ICacheStore<InMoq>>>();
                    Expression<Func<ICacheStore<InMoq>, ValueTask<Result>>> setup = x =>
                        x.RefreshAsync(It.Is<string>(s => s == EntryKey), It.IsAny<ICacheStoreOperationOptions>(),
                            It.IsAny<CancellationToken>());
                    mock.Setup(setup)
                        .ReturnsAsync(Result)
                        .Verifiable();
        ...
        public void RemovesInStore()
        {
            Container
                .Effect(c => WithExactOperationsCount(() =>
                {
                    ...
                    var mock = c.GetRequiredService<Mock<ICacheStore<InMoq>>>();
                    Expression<Func<ICacheStore<InMoq>, Result>> setup = x =>
                        x.Remove(It.Is<string>(s => s == EntryKey), It.IsAny<ICacheStoreOperationOptions>());
                    mock.Setup(setup)
                        .Returns(Result)
                        .Verifiable();
                    var cache = c.GetRequiredService<ICache<VoidUnit, InMoq>>();
        ...
        [Test]
        public Task RemovesInStoreAsync()
        {
            return Container
                .EffectAsync(c => WithExactOperationsCountAsync(async () =>
                {
                    ...
                    var mock = c.GetRequiredService<Mock<ICacheStore<InMoq>>>();
                    Expression<Func<ICacheStore<InMoq>, ValueTask<Result>>> setup = x =>
                        x.RemoveAsync(It.Is<string>(s => s == EntryKey), It.IsAny<ICacheStoreOperationOptions>(),
                            It.IsAny<CancellationToken>());
                    mock.Setup(setup)
                        .ReturnsAsync(Result)
                        .Verifiable();
